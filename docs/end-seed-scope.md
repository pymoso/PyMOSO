...
| docopt silent mis-parsing (issue 6) | `test_cli.py`/`test_cli_validation.py` (live parser contract) — argument-parsing logic, orthogonal to RNG consumption entirely. (`test_cli_characterization.py`'s frozen docopt behavior was independent evidence too, until it was deleted as an undeclared test-time dependency; see `KNOWN_ISSUES.md` issue 6.) | No |
